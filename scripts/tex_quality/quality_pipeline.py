...
def process_file(
    ...
    for idx, line in enumerate(lines):
        editable, in_math_block = is_prose_line(line, in_math_block)
        if not editable:
            continue

        original_editable_chunks.append(line)

        candidate = line
        candidate, term_hits = apply_termbase_preferred_forms(candidate, term_rules)
        candidate = run_languagetool_if_available(candidate, language)

        if candidate == line:
            accepted_editable_chunks.append(line)
            continue

        score = semantic_score_out_of_10(line, candidate)
        ...
        if accepted:
            updated_lines[idx] = candidate
            accepted_editable_chunks.append(candidate)
        else:
            accepted_editable_chunks.append(line)
