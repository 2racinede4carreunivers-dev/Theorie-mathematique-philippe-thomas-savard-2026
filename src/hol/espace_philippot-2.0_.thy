...
text \<open>
 ...
 en analogie avec les quaternions et la spirale de Theodore de Cyrene.
\<close>

typedecl event (* type abstrait pour les evenements *)
typedecl index (* type abstrait pour indexer les disques / niveaux *)

consts
 r :: "index \<Rightarrow> real" (* rayon du disque a l'etage n *)
 a :: "index \<Rightarrow> real" (* 4 parametres hypercomplexes *)
 b :: "index \<Rightarrow> real"
 c :: "index \<Rightarrow> real"
 d :: "index \<Rightarrow> real"
 V_pyr :: "index \<Rightarrow> real" (* volume de la pyramide *)
 V_ell :: "index \<Rightarrow> real" (* volume de l'ellipsoide associe *)
 val_geom :: "index \<Rightarrow> real" (* valeur geometrique caracteristique *)
 spiral_pos :: "index \<Rightarrow> event \<Rightarrow> real \<times> real \<times> real"
 (* position d'un evenement sur la spirale (espace-temps abstrait) *)


text \<open>
 Axiomes globaux liant spirale, valeurs geometriques, norme hypercomplexe,
 rapport de volumes et position des evenements.
\<close>
