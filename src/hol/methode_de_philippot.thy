...
section \<open>Geometrie du spectre premier - methode de Philippot.\<close>

text \<open>
 Ce fichier contient les definitions, structures et mecanismes necessaires
 a l'etude de la geometrie du spectre premier, incluant la methode de Philippot.
\<close>

subsection \<open>Etape 1 : suites explicites pour 3 a 11 termes\<close>
...
subsection \<open>Structure reglementaire des suites a l'etape 1\<close>
...
subsection \<open>Regle generale de substitution (toutes etapes \<ge> 2)\<close>

text \<open>
 La position de substitution depend uniquement du nombre de termes n :
 - pour 3 \<le> n \<le> 7, la position de substitution est n - 2 (1, 2, 3, 4, 5) ;
 - pour n \<ge> 8, la position de substitution est fixee a 6.
\<close>
...
subsection \<open>Etape 2 : suites explicites pour 3 a 7 termes\<close>
...
subsection \<open>Etape 3 : suites explicites pour 7 termes et moins\<close>

text \<open>
 L'etape 3 repete le mecanisme de l'etape 2 :
 une position est substituee, et une valeur compensatoire est ajoutee
 de l'autre cote de l'egalite. La position de substitution est la meme
 que pour l'etape 2 pour un nombre de termes donne.
\<close>
...
subsection \<open>Etape 3 : suites explicites pour 8 termes et plus\<close>
...
subsection \<open>Propriete fondamentale des puissances de deux\<close>
...
subsection \<open>Structure spectrale generale pour n termes et infinite detapes\<close>
...
text \<open>
 Propriete spectrale locale : pour tout i \<ge> 1,
...
text \<open>
 Interpretation :
 - la definition \<open>terme_spectral\<close> encode une infinite de termes 1 / 2^i, i \<ge> 1 ;
 - le lemme \<open>ratio_spectral_local\<close> montre que le rapport entre deux termes consecutifs
 est toujours 1/2, pour tout i ;
 - en prolongeant i sans borne, on obtient une structure spectrale qui produit 1/2
 une infinite de fois, de maniere purement arithmetique.
\<close>
